...
_declspec(dllexport) int fn_NetSim_LTE_Init()
{
	...
	{
		#ifdef DRX_ENABLE
		if (!strcmp(DEVICE(i + 1)->type, "ENB"))
		{
			pstruEventDetails->nDeviceId = i + 1;
			pstruEventDetails->dEventTime += 10 * MILLISECOND;
			pstruEventDetails->dPacketSize = 0;
			pstruEventDetails->nEventType = TIMER_EVENT;
			pstruEventDetails->nPacketId = 0;
			pstruEventDetails->nSubEventType = DRX_CHECK_BUFFER_STATUS;
		...
		#endif // DRX_ENABLE
		//set initial radio state for all ue's may be ACTIVE
		

		if(!strcmp(DEVICE(i + 1)->type, "UE"))
		{
			LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(i + 1, 1);
			fn_NetSim_LTE_DRX_Energy_configure(i+1);
			fn_NetSim_LTE_ChangeRadioState(i+1, DRX_ACTIVE, DRX_ACTIVE);
		}
	...
	case TIMER_EVENT:
		{
			...
			case DRX_TIMER_ON:
			{
				LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, 1);
				if (UEPhy->inactive_timer <= pstruEventDetails->dEventTime)
				{
					if (UEPhy->sleepCycleCounter == 3)//UE needs to be in short sleep atleast 3 time before entering long sleep
					{
						fn_NetSim_LTE_ChangeRadioState(pstruEventDetails->nDeviceId, UEPhy->nOldState, DRX_SLEEP);
						UEPhy->sleep_timer = pstruEventDetails->dEventTime + 10 * DRX_SUBFRAME_DURATION;
						UEPhy->sleepCycleCounter = 0;
						pstruEventDetails->nDeviceId = pstruEventDetails->nDeviceId;
						pstruEventDetails->dEventTime = UEPhy->sleep_timer;
						pstruEventDetails->dPacketSize = 0;
						pstruEventDetails->dPacketSize = 0;
						pstruEventDetails->nEventType = TIMER_EVENT;
						pstruEventDetails->nPacketId = 0;
						pstruEventDetails->nSubEventType = DRX_LONG_SLEEP;
						pstruEventDetails->pPacket = NULL;
						pstruEventDetails->nProtocolId = MAC_PROTOCOL_LTE;
						fnpAddEvent(pstruEventDetails);
					}
					...
					{
						fn_NetSim_LTE_ChangeRadioState(pstruEventDetails->nDeviceId, UEPhy->nOldState, DRX_SLEEP);
						UEPhy->sleep_timer = pstruEventDetails->dEventTime + 3 * DRX_SUBFRAME_DURATION;
						//Add timer event
						pstruEventDetails->nDeviceId = pstruEventDetails->nDeviceId;
						pstruEventDetails->dEventTime = UEPhy->sleep_timer;
						pstruEventDetails->dPacketSize = 0;
						...
						pstruEventDetails->nEventType = TIMER_EVENT;
						pstruEventDetails->nPacketId = 0;
						pstruEventDetails->nSubEventType = DRX_SHORT_SLEEP;
						pstruEventDetails->pPacket = NULL;
						pstruEventDetails->nProtocolId = MAC_PROTOCOL_LTE;
						fnpAddEvent(pstruEventDetails);
					}
					...
					{
						fn_NetSim_LTE_ChangeRadioState(pstruEventDetails->nDeviceId, UEPhy->nOldState, DRX_ACTIVE);
						UEPhy->sleepCycleCounter = 0;
				...
				break;
			case DRX_SHORT_SLEEP:
			{
				
				LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, 1);										
				//set UE state to long sleep
				fn_NetSim_LTE_ChangeRadioState(pstruEventDetails->nDeviceId, UEPhy->nOldState, DRX_SLEEP);
				UEPhy->onDuration_timer = pstruEventDetails->dEventTime + 2 * DRX_SUBFRAME_DURATION;
				UEPhy->sleepCycleCounter++;
				//Add timer event
				pstruEventDetails->nDeviceId = pstruEventDetails->nDeviceId;
				pstruEventDetails->dEventTime = UEPhy->onDuration_timer;
				pstruEventDetails->dPacketSize = 0;
				pstruEventDetails->dPacketSize = 0;
				pstruEventDetails->nEventType = TIMER_EVENT;
				pstruEventDetails->nPacketId = 0;
				pstruEventDetails->nSubEventType = DRX_TIMER_ON;
				pstruEventDetails->pPacket = NULL;
				pstruEventDetails->nProtocolId = MAC_PROTOCOL_LTE;
				fnpAddEvent(pstruEventDetails);
				...
				break;
			case DRX_INACTIVITY_TIMER_EXPIRE:
			{
			...
			case DRX_LONG_SLEEP:
			{
				LTE_ENB_MAC* enbMac = (LTE_ENB_MAC*)DEVICE_MACVAR(pstruEventDetails->nDeviceId, 1);
				LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, 1);
				LTE_ENB_PHY* phy = (LTE_ENB_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, 1);
				LTE_ASSOCIATEUE_INFO* info = enbMac->associatedUEInfo;
				if (UEPhy->onDuration_timer == pstruEventDetails->dEventTime)
				{
					//set UE state to active
					fn_NetSim_LTE_ChangeRadioState(pstruEventDetails->nDeviceId, UEPhy->nOldState, DRX_ACTIVE);
					UEPhy->sleep_timer = pstruEventDetails->dEventTime + 1 * phy->dSubFrameDuration;
					//Add timer event
					pstruEventDetails->nDeviceId = info->nUEId;
					pstruEventDetails->dEventTime = UEPhy->sleep_timer;
					pstruEventDetails->dPacketSize = 0;
					pstruEventDetails->dPacketSize = 0;
					pstruEventDetails->nEventType = TIMER_EVENT;
					pstruEventDetails->nPacketId = 0;
					pstruEventDetails->nSubEventType = PhysicalChannel_PDCCH;
					pstruEventDetails->pPacket = NULL;
					pstruEventDetails->nProtocolId = MAC_PROTOCOL_LTE;
					fnpAddEvent(pstruEventDetails);
				}
			...
			case DRX_CHECK_BUFFER_STATUS:
			{
				...
				while (info)
				{
					LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(info->nUEId, 1);
						//UEPhy->inactive_timer = pstruEventDetails->dEventTime + 3 * phy->dSubFrameDuration;
						if (info->GBRQueue->nSize == 0 && info->nonGBRQueue->nSize == 0)
						{
							//Add timer event
							pstruEventDetails->nDeviceId = info->nUEId;
							pstruEventDetails->dEventTime += 3 * phy->dSubFrameDuration;
							pstruEventDetails->dPacketSize = 0;
							pstruEventDetails->dPacketSize = 0;
							pstruEventDetails->nEventType = TIMER_EVENT;
							pstruEventDetails->nPacketId = 0;
							pstruEventDetails->nSubEventType = DRX_INACTIVITY_TIMER_EXPIRE;
							pstruEventDetails->pPacket = NULL;
							pstruEventDetails->szOtherDetails = NULL;
							pstruEventDetails->nProtocolId = MAC_PROTOCOL_LTE;
							fnpAddEvent(pstruEventDetails);
...
{
	NETSIM_ID nLoop;
	ptrLTE_UE_PHY phy = NULL;
	
	for (nLoop = 0; nLoop < NETWORK->nDeviceCount; nLoop++)
	{
		if (!strcmp(DEVICE(nLoop + 1)->type, "UE"))
		{
			phy = (LTE_UE_PHY*)DEVICE_PHYVAR(nLoop + 1, 1);		
			fn_NetSim_LTE_ChangeRadioState(nLoop+1, phy->nOldState, phy->nNewState);
		}

	}
	battery_metrics(file);
