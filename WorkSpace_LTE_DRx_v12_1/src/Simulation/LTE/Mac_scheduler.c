...
int fn_NetSim_LTE_AddInQueue()
{
	while(fn_NetSim_GetBufferStatus(DEVICE_INTERFACE(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->pstruAccessInterface->pstruAccessBuffer))
	{
		...
		LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, pstruEventDetails->nInterfaceId);
		//RRC protocol
...
{
	LTE_ENB_PHY* phy = (LTE_ENB_PHY*)DEVICE_PHYVAR(pstruEventDetails->nDeviceId, pstruEventDetails->nInterfaceId);	
	while(fn_NetSim_GetBufferStatus(DEVICE_INTERFACE(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->pstruAccessInterface->pstruAccessBuffer))
	{
		NetSim_PACKET* packet=fn_NetSim_Packet_GetPacketFromBuffer(DEVICE_INTERFACE(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->pstruAccessInterface->pstruAccessBuffer,1);
		LTE_QUEUE* queue;
		LTE_UE_PHY* UEPhy = (LTE_UE_PHY*)DEVICE_PHYVAR(get_first_dest_from_packet(packet), 1);
		...
		UEPhy->inactive_timer = pstruEventDetails->dEventTime + 10 * DRX_SUBFRAME_DURATION;
	}
	return 1;
}
...
static int fn_NetSim_LTE_FormDownlinkFrame_RoundRobin(NETSIM_ID enbId,NETSIM_ID enbInterface,unsigned int carrier_id,unsigned int nSSId)
{
	...
	while(info)
	{
		...
		if((info->carrier_id==-1 || info->carrier_id==(int)carrier_id) &&
			info->nSSId == nSSId)
		{
			if(info->rrcState==RRC_CONNECTED && info->GBRQueue->nSize && UEPhy->nRadioState!=DRX_SLEEP)
			{
				unsigned int index=0;
	...
	while(info)
	{
		...
		if((info->carrier_id==-1 || info->carrier_id == (int)carrier_id) &&
			info->nSSId == nSSId)
		{
			if(info->rrcState==RRC_CONNECTED && info->nonGBRQueue->nSize && UEPhy->nRadioState != DRX_SLEEP)
			{
				unsigned int index=0;
