...
bool fn_NetSim_LTE_ChangeRadioState(NETSIM_ID nDeviceId, LTE_DRX_STATE nOldState, LTE_DRX_STATE nNewState)
{
	ptrLTE_UE_PHY phy = (LTE_UE_PHY*)DEVICE_PHYVAR(nDeviceId, 1);
...
int fn_NetSim_LTE_DRX_Energy_configure(NETSIM_ID nDeviceId) {
	LTE_UE_PHY *phy = (LTE_UE_PHY*)DEVICE_PHYVAR(nDeviceId, 1);
	phy->inactive_timer = 0;
	phy->sleep_timer = 0;
	phy->sleepCycleCounter = 0;
	phy->onDuration_timer = 0;

	double dRechargingCurrent = 0;
	double dVoltage = 3.6;
	double dInitialEnergy = 10;
	...
	double dCurrent = 3.4;
	battery_add_new_mode(phy->battery, DRX_ACTIVE, dCurrent, "Active energy(mJ)");

 	dCurrent = 1.4;
	battery_add_new_mode(phy->battery, DRX_SLEEP, dCurrent, "Sleep energy(mJ)");
